...
**_Note_**:
This GitHub repository contains code for a demonstration application
that is non-proprietary. This is a subset of project that includes proprietary code.

In the links below only links to the demoapp project are functional. Links to other projects
would lead to proprietary code that is not included in this repository. The the links have
been left in place in this README to provide historical context.
...
- [The mocksysteundertest Demonstration Container](https://github.com/hagdog/containerized-demo-app/blob/master/redacted.md) --
  a functional description of the demoapp application.
- [mobius TestAssitant](https://github.com/hagdog/containerized-demo-app/blob/master/redacted.md) --
  TestAssitant overview including build and test instructions.
- [Sidecar Mediator API](https://github.com/hagdog/containerized-demo-app/blob/master/redacted.md) --
...
| Project    | File                                                                                                            | Info                                                                                                                                                   |
| ---------- | --------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------- |
| demoapp    | [sidecarinterface.py](https://github.com/hagdog/containerized-demo-app/blob/master/demoapp/sidecarinterface.py) | The `send_ready_update()` function creates the standby and ready messages.                                                                             |
| demoapp    | [seerpsyche.py](https://github.com/hagdog/containerized-demo-app/blob/master/demoapp/seerpsyche.py)             | State transitions use `notifier.send_ready_update()` to update the sidecar with readiness messages.                                                    |
| testdriver | [test_sidecar.py](https://github.com/hagdog/containerized-demo-app/blob/master/redacted.md))                    | The `wait_for_ready()` and `wait_for_stop()` functions in the SidecarMediator are used frequently with start and stop operations on the SUT container. |
| testdriver | [test_sidecar_injection.py](https://github.com/hagdog/containerized-demo-app/blob/master/redacted.md))          | Ready messages are cached on a filesystem and read by the application when it starts.                                                                  |
...
| testdriver      | [test_sidecar.py](https://github.com/hagdog/containerized-demo-app/blob/master/redacted.md))                                 | `test_reset()`                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Project         | File                                                                                                      | Info                                                                                                                                                                                                                        |
| --------------- | --------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| sidecarmediator | [README.md](https://github.com/hagdog/containerized-demo-app/blob/master/redacted.md)                     | See `send_signal()`.                                                                                                                                                                                                        |
| demoapp         | [sidecarinterface.py](https://github.com/hagdog/containerized-demo-app/blob/master/demoapp/seerpsyche.py) | SIGTERM: In an orderly shutdown, the seers "memories" are saved in the `Seer._handle_shutdown_signal() function`. The memories are recovered at startup in `Seer.__init__()`                                                |
| demoapp         | [seerpsyche.py](https://github.com/hagdog/containerized-demo-app/blob/master/demoapp/seerpsyche.py)       | Other signals are used to manage state transistions and configuration updates in demoapp. See `_register_event_signals`(), `_register_shutdown_signal()`, `register_service_interface_shutdown()`, `_handle_event_signal()` |
| testdriver      | [test_sidecar.py](https://github.com/hagdog/containerized-demo-app/blob/master/redacted.md)               | `test_signal_sut()`, Stop testing indirectly tests SIGTERM since Docker sends SIGTERM when stopping a container.                                                                                                            |
...
| Project | File                                                                                                            | Info                                              |
| ------- | --------------------------------------------------------------------------------------------------------------- | :------------------------------------------------ |
| demoapp | [docker-compose.yaml](https://github.com/hagdog/containerized-demo-app/blob/master/demoapp/docker-compose.yaml) | A volume is mounted where the logs are collected. |
...
| Project    | File                                                                                                                                                                              | Info                                                                                      |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------- |
| demoapp    | [sidecarinterface.py](https://github.com/hagdog/containerized-demo-app/blob/master/demoapp/sidecarinterface.py)                                                                   | `send_injected_messages` processes messages on a filesystem and sends them to the sidecar |
